...
resource "aws_autoscaling_group" "app1_asg_80-tokyo" {
  ...
  force_delete               = true
  target_group_arns          = [aws_lb_target_group.app1_tg_80-tokyo.arn]

  launch_template {
    id      = aws_launch_template.app1_LT_80-tokyo.id
    version = "$Latest"
  }
...
resource "aws_autoscaling_policy" "app1_scaling_policy_80-tokyo" {
  ...
  autoscaling_group_name = aws_autoscaling_group.app1_asg_80-tokyo.name
...
resource "aws_autoscaling_attachment" "app1_asg_80_attachment-tokyo" {
  autoscaling_group_name = aws_autoscaling_group.app1_asg_80-tokyo.name
  lb_target_group_arn   = aws_lb_target_group.app1_tg_80-tokyo.arn
}